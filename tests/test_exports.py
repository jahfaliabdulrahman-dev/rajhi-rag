...
def _synthetic_run(tmp_path: Path) -> Path:
    ...
    (run / "results" / "pg-001.json").write_text(json.dumps({
        "pg": 1, "page_no": 1,
        "raw_rows": [{"movement": "300.00", "balance": "100.00", "desc": "تحويل",
                      "date": "٢٠٢٤٠١٠١", "raw_movement": "٣٠٠,٠٠",
                      "raw_balance": "١٠٠,٠٠"}],
    ...
    (run / "slice_report.json").write_text(json.dumps({
        ...
        "totals": {"rows": 1, "clean": 1, "clean_ratio": 1.0},
        "usage": {"cost": 0.0123, "calls": 2},
        "footer": {"ok": 1, "mismatch": 0, "absent": 1, "unchecked": 0, "gap": 0},
        "page_numbers": {"gaps": []},
        "per_page": [
            {"page": 1, "rows": 1, "footer": "ok", "suspects": 0, "page_no": 1,
             "origin": "live"},
...
def test_xlsx_has_the_four_sheets(tmp_path):
    out = tmp_path / "export.xlsx"
    info = build(_synthetic_run(tmp_path), out, None)
    wb = load_workbook(out)
    assert wb.sheetnames == ["الحركات", "التحقق لكل صفحة", "ما لم يُثبت",
                             "كيف تُقرأ هذه الأوراق"]
    assert info["rows"] == 1 and info["pages"] == 2
