...
#: رسم المستودع. **والتركيبُ مُقاسٌ لا موصوف**: ٤٧ سطراً منها تُجيزها كلمةُ `commit` (ترويساتُ رسائلَ
#: قديمة: `commit: <hex>` تشير إلى التزاماتٍ **ما قبل التنقية**) وواحدٌ بكلمة `sha` وحدها ⇒ فهو دَينُ
#: الصنفِ الذي يطارده P-4 لا ضجيجَ بصماتِ ملفّ (تلك **لا تعبر البوّابةَ أصلًا** بعد R56-1ب).
#: **والسقفُ راتشتٌ في الاتجاهين** (`==` لا `<=`): الصعودُ يقول «انكشافٌ جديد» والنزولُ يقول «كشفٌ مفقود»
#: — وكلاهما يستحقّ توقّفاً، لا أن يُبتلع صامتاً. (تاريخُه: ٨٥ ⇒ ٦٣ ⇒ ٦٤ ⇒ ٥٠ ⇒ ٤٨.)
FOREIGN_ID_DEBT_ON_CORPUS = 48
...
def test_the_four_measured_forms_of_the_pack_stamp_are_not_blocked():
    ...
    إصلاحُ «كلمةِ هاشٍ في ±٦٠ خانةً» **يُبقيه محجوباً** (قاسه المقعدان). والبوّابةُ الجديدة (`\bsha\b`)
