...
final row shaping identical to app.process_pdf), then reports:
  ...
  2. every row with movement == 1000.00            (owner's four-row question)
...
def main() -> None:
    ...
    print("\n=== [2] ROWS WITH AMOUNT == 1000.00 ===")
    hit = 0
    for r in rows:
        mv = r.get("movement")
        if isinstance(mv, Decimal) and mv == Decimal("1000.00"):
            hit += 1
            print(f"row {r['row_no']:3d} | صفحة {r['page']:2d} | "
                  f"side={r['side'] or '—':6s} | balance={r['balance']} | "
                  f"desc={str(r.get('desc'))[:95]}")
    print(f"-> {hit} row(s) with 1000.00")
