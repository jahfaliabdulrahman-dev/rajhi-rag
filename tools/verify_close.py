...
def main() -> int:
    ...
    for g in gaps:
        ...
        # اسم القيد يتبع **الورق** لا موقعاً قد يكون فارغاً: بعد أن صار عمود
        # «الصفحة (ملف)» فارغاً لصفوف الفجوة (لا صفحة ملف تُتّهم)، يُسمّى القيد
        # بأرقام الأوراق الغائبة المطبوعة — وإلا طبعت البوابة «قيد صNone».
        label = g.get("الصفحة (ملف)") or g.get("رقم الصفحة") or "?"
        check(f"قيد ص{label} له مقدارا مدين ودائن",
