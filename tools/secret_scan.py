...
#: ⇒ الدقّةُ تُنقَل إلى **البوّابة** لا إلى الاستثناء: سياقُ الالتزام يشترط كلمةَ `sha` **وحدَها** (`\bsha\b`)
...
ID_CONTEXT = re.compile(r"(?i)(\bsha\b|commit|\u0627\u0644\u062a\u0632\u0627\u0645|revision|\brev\b|/commit/)")
...
def scan_foreign_ids(lines: list[tuple[int, str]], exists=_exists_in_repo) -> list[tuple[int, str]]:
    ...
    for n, text in lines:
        ...
        for match in HEXISH.finditer(text):
            ...
            # البوّابةَ أصلًا بفضل `\bsha\b` ⇒ لا حاجةَ إلى نافذةٍ ولا معجمٍ (حركةُ الجودو).
