...
def main() -> None:
    ...
    if d is None:
        ...
        live = _test_count()
        if live is None:
            # **الفشلُ مُغلَق (R67-2 · مراجعة ٦٧):** كان التخطّي صامتًا حين لا `pytest` في البيئة —
            # وخطوةُ الـCI كانت تجري **قبل** تثبيته ⇒ فالقياسُ الحيُّ لم يجرِ قطُّ في الـCI، والحكمُ
            # «توافق» يعني «لقطةٌ توافق وثيقةً متقادمةً مثلها». رقمٌ لا يُقاس لا يُصادَق عليه.
